...
class Role(db.Model):
    ...
    def update_roles():
        ...
        """
        roles = {'User': (Permission.FOLLOW | Permission.COMMENT | Permission.WRITE_ARTICLES, True), 'Moderator': (
...
class User(UserMixin, db.Model):
    ...
    def __init__(self, **kwargs):
        ...
        if self.email is not None and self.avatar_path is None:
            self.avatar_path = '/uploaded/default_man_head.png/'
    ...
    avatar_path = db.Column(db.String(128))  # 头像的path
    ...
    followed = db.relationship('Follow', foreign_keys=[Follow.follower_id],  # 在 Follow 模型中自引用
                               ...
                               lazy='dynamic', cascade='all, delete-orphan')  # 删除所有记录
    # 用户的粉丝<关系>
    followers = db.relationship('Follow', foreign_keys=[Follow.followed_id],  # 在 Follow 模型中自引用
    ...
    def generate_fake_users(count=100):
        ...
        for i in range(count):
            ...
            u = User(email=_email, username=forgery_py.internet.user_name(), password=forgery_py.lorem_ipsum.word(),
                about_me=forgery_py.lorem_ipsum.sentence(), confirmed=True,
                avatar_path=hashlib.md5(_email.encode('utf-8')).hexdigest(), member_since=forgery_py.date.date(True), )
            db.session.add(u)
            try:
                db.session.commit()
    ...
    def to_json(self, posts):
        return {'username': self.username, 'memberSince': self.member_since, 'lastSeen': self.last_seen,
            'postCount': self.posts.count(), 'posts': posts}
...
class Post(db.Model):
    ...
    def to_json(self):
        return {'posTime': self.timestamp, 'post': self.body, 'authorID': self.author_id}
...
class Comment(db.Model):
    ...
    def to_json(self):
        return {'postTime': self.timestamp, 'post': self.body, 'postID': self.post_id, 'authorID': self.author_id}
